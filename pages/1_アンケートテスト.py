...
if user_id and bmr:
    ...
    if not st.session_state["questions_locked"]:
        ...
        q1 = st.radio("普段から健康に気を使っていますか？", ["はい", "どちらともいえない", "いいえ"], key="temp_q1")
        q2 = st.text_input("1日に何食食べますか？", key="temp_q2")
        q3 = st.text_input("間食をする頻度は？", key="temp_q3")
        q4 = st.radio("栄養バランスを意識して食事を選びますか？", ["はい", "どちらともいえない", "いいえ"], key="temp_q4")
        q5 = st.radio("ダイエット経験はありますか？", ["はい", "どちらともいえない", "いいえ"], key="temp_q5")

        st.header("健康・体型知識について")
        know_bmr = st.radio("「基礎代謝」という言葉を知っていますか？", ["はい", "いいえ"], key="temp_know_bmr")
        know_bmi = st.radio("「BMI」という言葉を知っていますか？", ["はい", "いいえ"], key="temp_know_bmi")

        bmr_q1 = bmr_q2 = bmi_q1 = bmi_q2 = ""
        if know_bmr == "はい":
            bmr_q1 = st.radio("基礎代謝が高いほど痩せやすい？", ["はい", "いいえ"], key="temp_bmr_q1")
            bmr_q2 = st.radio("基礎代謝は年齢とともに下がる？", ["はい", "いいえ"], key="temp_bmr_q2")

        if know_bmi == "はい":
            bmi_q1 = st.radio("BMIは身長と体重から計算される？", ["はい", "いいえ"], key="temp_bmi_q1")
            bmi_q2 = st.radio("BMIが22だと病気になりにくい？", ["はい", "いいえ"], key="temp_bmi_q2")
